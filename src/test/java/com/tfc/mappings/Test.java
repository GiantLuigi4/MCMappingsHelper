...
public class Test {
	public static void main(String[] args) throws IOException {
		for (int i = 14; i <= 16; i++) {
			...
			File f = new File("com.tfc.mappings\\inter\\1." + i + "_formatted.txt");
			...
			f = new File("com.tfc.mappings\\inter\\1." + i + "_phantom_class.txt");
			f.getParentFile().mkdirs();
			f.createNewFile();
		...
		File f = new File("com.tfc.mappings\\mojmap\\1.16_formatted.txt");
		f.getParentFile().mkdirs();
		f.createNewFile();
		FileOutputStream stream = new FileOutputStream(f);
		assert holder1 != null;
